...
float aspect; //长宽比，如果要调整窗口大小则需要调整
bool fullscreen;
bool saveOutput; //确保存贮你的动画
...
float sph_rad;
int sph_vert, env_vert; // number of vertices for sphere and environment，小球的环境和顶点数量

//Index of where to model, view, and projection matricies are stored on the GPU,GPU上存储模型，视图和投影矩阵的位置的索引
GLint uniModel, uniView, uniProj, uniColor;

// user interaction variables,用户交互变量
bool grabbed;
...
int main(int argc, char* argv[]) {
    ...
    //======================= Initializations and Window Setup =================================，初始化及窗口设置

    SDL_Init(SDL_INIT_VIDEO);  //Initialize Graphics (for OpenGL)，初始化图形

    //Ask SDL to get a recent version of OpenGL (3.2 or greater)，让SDL获取一个最近版本的OpenGL3.2以上
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 3);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 2);

    //Create a window (offsetx, offsety, width, height, flags)利用创建一个窗口
    ...
    //The above window cannot be resized which makes some code slightly easier.，为了是代码更简洁一些，无法调整窗口的大小
    //Below show how to make a full screen window or allow resizing，如果想要调整大小，按如下所示操作
    ...
    //============================ Shader Setup ======================================，设置shader

    //Load the default vertex Shader,加载默认顶点shader
    GLuint vertexShader = glCreateShader(GL_VERTEX_SHADER);
    loadShader(vertexShader, "../ParticleSystems/Shader/vertexshader.txt");

    //Load the vertex Shader for particles，加载粒子顶点着色器
    ...
    //Load the default fragment Shader ，加载默认的片元着色器
    GLuint fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    loadShader(fragmentShader, "../ParticleSystems/Shader/fragmentshader.txt");

    //Load the fragment Shader for particles 加载粒子片元着色器
    ...
    //Join the vertex and fragment shaders together into one program 链接顶点着色器和片元着色器
    ...
    loadobj("../ParticleSystems/Assets/stones.obj", vertices, uvs, normals); // append to the vector (does not matter since both objects are using the same format)，两个对象的格式相同
    env_vert = vertices.size();

    //============================ Buffer Setup ====================================== 设置缓存

    //Build a Vertex Array Object. This stores the VBO and attribute mappings in one object，创建一个VAO，这会将VBO和属性映射存储在一个对象中
    GLuint vao, vao_sph;
    glGenVertexArrays(1, &vao); // VAO for particles
    glGenVertexArrays(1, &vao_sph); // VAO for the sphere and stones

    glBindVertexArray(vao); //Bind the above created VAO to the current context，绑定上面创造的VAO到上下文

    //Allocate memory on the graphics card to store geometry (vertex buffer object)在图形卡上分配内存以存储几何图形（顶点缓冲区对象）
    GLuint vbo;
    glGenBuffers(1, &vbo);  //Create 1 buffer called vbo，创建顶点缓存对象

    glBindBuffer(GL_ARRAY_BUFFER, vbo); //Set the vbo as the active array buffer (Only one buffer can be active at a time)
    glBufferData(GL_ARRAY_BUFFER, sizeof(ptc_vert), ptc_vert, GL_STATIC_DRAW); //upload vertices to vbo，传递数据
    //GL_STATIC_DRAW means we won't change the geometry, GL_DYNAMIC_DRAW = geometry changes infrequently
    //GL_STREAM_DRAW = geom. changes frequently.  This effects which types of GPU memory is used

    //Tell OpenGL how to set fragment shader input (for particles)，让opengl知道如何去设置片元着色器输入
